(() => {
  ...
  function visibleSources() {
    ...
    return (data().sources || []).filter((source) => {
      ...
      const haystack = `${source.name} ${source.family} ${source.access} ${source.detail} ${(source.records || []).map((record) => `${record.name || ''} ${record.ticker || ''} ${record.form || ''} ${record.group || ''}`).join(' ')}`.toLowerCase();
  ...
  function recordCard(record) { if (record.kind === 'filing') return filingCard(record); if (record.kind === 'release') return releaseCard(record); return seriesCard(record); }
