(() => {
  ...
  document.querySelectorAll('[data-shell-view]').forEach((button) => {
    button.addEventListener('click', () => goToRoute(button.dataset.shellView));
  });

  moreButton?.addEventListener('click', () => {
    if (moreDrawer?.hidden) openMore();
    else closeMore();
  });
  ...
  railToggle?.addEventListener('click', () => {
    ...
    try { localStorage.setItem('marketBriefRailExpanded', expanded ? '1' : '0'); } catch (_) { /* storage is optional */ }
  });
