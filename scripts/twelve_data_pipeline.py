...
def previous_by_id(previous: dict[str, Any]) -> dict[str, dict[str, Any]]:
    return {
        ...
        for item in previous.get("watchlist", [])
...
def provider_block(status: str) -> dict[str, Any]:
    return {
        ...
        "access": "credentialed-server-side",
        "licenseMode": "private-internal-use-only",
        "status": status,
        "disclaimer": (
            "Provider data is collected by GitHub Actions for an access-controlled "
            "internal dashboard. Do not enable while the repository or deployed site "
...
def disabled_payload(items: list[dict[str, Any]], *, now: datetime, reason: str) -> dict[str, Any]:
    ...
    return {
        ...
        "provider": provider_block("unavailable"),
        "collection": {
            "mode": "disabled",
            "status": "unavailable",
            "expectedCadence": "Intraday snapshots during the US session and one daily history refresh",
        ...
        "sourceStatus": [
            {
                "id": "twelve-data-api",
                "source": "Twelve Data private market feed",
                "status": "unavailable",
        ...
        "methodology": {
            "price": "Latest accepted quote close, otherwise latest accepted daily close.",
            "returns": "Trading-session comparisons using 1, 5, 21, 63 and 252-session references where available.",
            "trend": "Rule-based price and 20/50/200-session moving-average alignment; not a recommendation.",
            "retention": "A failed symbol retains its last verified row with stale status rather than being erased.",
        },
    }
...
def collect_payload(
    ...
    *,
    ...
    for item in items:
        ...
        fresh_response = quote is not None or history_refreshed
        if fresh_response:
            record = build_record(
                item,
                quote=quote,
                history=history,
                now_iso=now_iso,
                partial_error="; ".join(item_errors) if item_errors else None,
            )
            ...
            record = stale_previous(
                item,
                old,
                now_iso=now_iso,
                error="; ".join(item_errors) or "No fresh provider response was accepted",
            )
    ...
    failure_count = len(output) - fresh_success_count
    if current_count == len(output):
        collection_status = "current"
    elif fresh_success_count:
        collection_status = "partial"
    else:
        collection_status = "failed"

    observed = max(
        (str(item.get("observedAt")) for item in output if item.get("observedAt")),
        default=None,
    )
    previous_last_success = previous.get("collection", {}).get("lastSuccessfulAt")
    last_success = now_iso if fresh_success_count else previous_last_success
    stale_count = sum(1 for item in output if item.get("status") == "stale")

    return {
        "schemaVersion": 1,
        "generatedAtUtc": now_iso,
        "provider": provider_block(collection_status),
        "collection": {
            "mode": mode,
